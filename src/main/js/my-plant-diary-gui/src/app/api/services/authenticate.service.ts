...
import { TokenSo } from '../models/token-so';
...
export class AuthenticateService extends BaseService {
   ...
   */
  static readonly AuthenticateUserPath = '/api/authenticate';
  ...
  authenticateUser$Response(params: {
    body: AuthSo
  }): Observable<StrictHttpResponse<TokenSo>> {

    const rb = new RequestBuilder(this.rootUrl, AuthenticateService.AuthenticateUserPath, 'post');
    if (params) {
    ...
    return this.http.request(rb.build({
      ...
      map((r: HttpResponse<any>) => {
        return r as StrictHttpResponse<TokenSo>;
      })
    );
  }
  ...
  authenticateUser(params: {
    ...
    return this.authenticateUser$Response(params).pipe(
      map((r: StrictHttpResponse<TokenSo>) => r.body as TokenSo)
    );
  }
